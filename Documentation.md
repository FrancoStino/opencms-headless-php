...
Questo progetto implementa un sito web headless basato su PHP che utilizza l'API JSON di OpenCMS per recuperare contenuti. Il sistema supporta rendering dinamico di pagine container, liste di contenuti e viste dettagliate, con supporto multilingua e dark mode.

### Scopo Principale
Creare un'alternativa PHP all'implementazione Next.js originale, mantenendo la stessa esperienza utente e funzionalità attraverso un'architettura MVC server-side.
